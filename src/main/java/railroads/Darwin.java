...
public class Darwin {
    ...
    private ArrayList<Agent> repopulateAgents(ArrayList<EvaluatedSolution> evaluatedSolutions, long seed){
        ...
        EvaluatedSolution parentB;
        for(int i = 0; !oldAgents.isEmpty(); i=(i+1)%oldAgents.size()){
            if(rand.nextDouble() > Settings.CROSS_SKIP) continue;
            var agent = oldAgents.get(i);
            if(parentA == null){
                parentA = agent;
                oldAgents.remove(i);
                continue;
            ...
            HashSet<Transform> maskB = parentA.flood().stream().map(Gene::getTransform).collect(Collectors.toCollection(HashSet::new));
            mask.addAll(maskB);
